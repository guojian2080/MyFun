...
- (NSMutableArray *)allAssetTypes{
    if (!_allAssetTypes) {
        ...
        NSArray *assetTypes = [[GJItemStore sharedStore] allAssetTypes];
        for (NSManagedObject *type in assetTypes) {
            [tmpArray addObject:type];
        }
        _allAssetTypes = tmpArray;
    }
    return _allAssetTypes;
}

- (IBAction)addNewAsset:(id)sender{
//    if (self.addAssetDelegate && [self.addAssetDelegate respondsToSelector:@selector(addAsset)]) {
//        [self.addAssetDelegate addAsset];
//    }
    [[GJItemStore sharedStore] createToAssetTypes:self.allAssetTypes];
    NSLog(@"%@", self.allAssetTypes);
...
- (instancetype) initWithStyle:(UITableViewStyle)style{
    if (self = [super initWithStyle:style]) {
        ...
        UIBarButtonItem *bbi = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self action:@selector(addNewAsset:)];
...
#warning Incomplete implementation, return the number of sections
    return [self.allAssetTypes count];
}
...
//    NSManagedObject *assetType = allAssets[indexPath.row];
    NSManagedObject *assetType = self.allAssetTypes[indexPath.row];
