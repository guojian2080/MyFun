...
{
    UIPanGestureRecognizer *gr = self.moveRecognizer;
    if (gr.state == UIGestureRecognizerStateChanged){
        CGPoint vector = [gr velocityInView:self];
        CGFloat velocity = sqrtf(vector.x * vector.x + vector.y * vector.y);
        NSLog(@"%f",velocity);
//        self.lineWidth = velocity / 2;
    }
...
//        NSLog(@"%@",NSStringFromCGPoint(line.begin));
//        NSLog(@"%@",NSStringFromCGPoint(line.end));
